...
/*   Updated: 2023/04/10 15:49:56 by crepou           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_left(mlx_key_data_t keydata, t_system *system)
{
		...
		(keydata.action == MLX_PRESS || keydata.action == MLX_REPEAT))
	{
		...
		if (elem->val == '0')
			move_over_empty_space(system, 8, x, y);
		else if (elem->val == 'C')
			move_over_collectible(system, 8);
...
void	move_right(mlx_key_data_t keydata, t_system *system)
{
		...
		(keydata.action == MLX_PRESS || keydata.action == MLX_REPEAT))
	{
		...
		elem = elem_identify(system->map, (x + 1), y);
		if (elem->val == '0')
			move_over_empty_space(system, 4, x, y);
		else if (elem->val == 'C')
			move_over_collectible(system, 4);
		else if (elem->val == 'E' && system->map->map_info->exit_enabled)
		{
			mlx_close_window(system->window);
			system->map->map_info->win = 0;
...
void	move_down(mlx_key_data_t keydata, t_system *system)
{
		...
		(keydata.action == MLX_PRESS || keydata.action == MLX_REPEAT))
	{
		...
		if (elem->val == '0')
			move_over_empty_space(system, 10, x, y);
		else if (elem->val == 'C')
			move_over_collectible(system, 10);
		else if (elem->val == 'E' && system->map->map_info->exit_enabled)
		{
			mlx_close_window(system->window);
			system->map->map_info->win = 0;
...
void	move_up(mlx_key_data_t keydata, t_system *system)
{
		...
		(keydata.action == MLX_PRESS || keydata.action == MLX_REPEAT))
	{
		...
		elem = elem_identify(system->map, x, y - 1);
		if (elem->val == '0')
			move_over_empty_space(system, 9, x, y);
		else if (elem->val == 'C')
			move_over_collectible(system, 9);
		else if (elem->val == 'E' && system->map->map_info->exit_enabled)
		{
			mlx_close_window(system->window);
			system->map->map_info->win = 0;
